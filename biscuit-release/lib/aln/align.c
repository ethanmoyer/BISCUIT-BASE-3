...
int usage(mem_opt_t *opt) {
    ...
    fprintf(stderr, "       -b INT        For PE, read 1 to parent, read 2 to daughter (1, directional library);\n");
    fprintf(stderr, "                     read1 and read 2 to both (0: default, nondirectional library); For SE, parent (1);\n");
    ...
    fprintf(stderr, "\nScoring options:\n");
    fprintf(stderr, "       -A INT        score for a sequence match, which scales options -TdBOELU\n");
    fprintf(stderr, "                     unless overridden [%d]\n", opt->a);
    fprintf(stderr, "       -B INT        penalty for a mismatch [%d]\n", opt->b);
    fprintf(stderr, "       -O INT[,INT]  gap open penalties for deletions and insertions [%d,%d]\n", opt->o_del, opt->o_ins);
    fprintf(stderr, "       -E INT[,INT]  gap extension penalty; a gap of size k cost\n");
    fprintf(stderr, "                     '{-O} + {-E}*k' [%d,%d]\n", opt->e_del, opt->e_ins);
    fprintf(stderr, "       -L INT[,INT]  penalty for 5'- and 3'-end clipping [%d,%d]\n", opt->pen_clip5, opt->pen_clip3);
    fprintf(stderr, "       -U INT        penalty for an unpaired read pair [%d]\n", opt->pen_unpaired);
    fprintf(stderr, "       -x STR        read type. Setting -x changes multiple parameters unless\n");
    fprintf(stderr, "                     overriden [null]\n");
    ...
    fprintf(stderr, "                     if absent), max (4 sigma from the mean if absent) and min\n");
    fprintf(stderr, "                     of insert size distribution.FR orientation only [inferred]\n");
    fprintf(stderr, "\n");
    free(opt);
    return 1;
}
